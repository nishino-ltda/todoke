...
*   **Fluxo Principal:**
    ...
    3.  Sistema valida os dados e envia email/SMS de verificação.
...
*   **Fluxo Principal:**
    ...
    5.  Sistema calcula o preço e tempo estimado, considerando distância, trânsito (se disponível) e fatores dinâmicos.
    ...
    8.  Sistema busca o entregador mais adequado (Aurora) baseado em proximidade, disponibilidade e tipo de veículo.
...
*   **Fluxo Principal:**
    ...
    4.  Sistema atualiza o status da entrega e notifica EcoCliente.
...
*   **Fluxo Principal:**
    ...
    6.  Gerente confirma a solicitação. Sistema processa e envia para o entregador (ex: Aurora via App Ionic).
...
*   **Fluxo Principal (Painel Web):**
    ...
    2.  AdminSys visualiza um Dashboard (`/admin/dashboard`) específico com métricas chave da plataforma: número de usuários ativos, entregas em andamento (`TdkStatWidget`, `TdkCard`).
    ...
    4.  AdminSys navega para aprovar/rejeitar novos registros de parceiros.
...
*   **Fluxo Alternativo:**
    *   Se um parceiro estiver offline, AdminSys pode tentar reativá-lo pelo painel ou contatar o parceiro.
...
*   **Fluxo Principal:**
    ...
    7.  O serviço de sincronização envia as atualizações pendentes para o servidor.
...
### [❌] 1.8 LogisMaster Gerencia Regiões via Painel Web

*   **Atores:** LogisMaster (Admin), AdminSys
*   **Descrição:** O administrador da LogisMaster utiliza o painel web (Laravel + Vue/Inertia) para gerenciar as regiões geográficas de operação. AdminSys supervisiona.
*   **Fluxo Principal:**
    ...
    6.  Sistema valida e armazena as informações. O sistema usa esses dados para roteamento geográfico.
    7.  AdminSys (em seu painel `/admin/regions`) pode visualizar as regiões de todos os parceiros, e tem a capacidade de aprovar/rejeitar novos parceiros ou intervir se necessário.
*   **Fluxo Alternativo:**
...
*   **Fluxo Principal:**
    ...
    2.  Sistema identifica que o destino é elegível para entrega via drone hub e que Aurora (ou o entregador alocado) tem preferência por não subir em apartamentos.
    ...
    8.  Sistema notifica a DroneExpress (via API) sobre a chegada do pacote e os detalhes da entrega final.
    9.  DroneExpress prepara e lança um drone com o pacote para o endereço final do EcoCliente.
    10. EcoCliente acompanha a etapa final da entrega (drone) pelo app Ionic.
    ...
    13. Sistema marca a entrega como concluída.
