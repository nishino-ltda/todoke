<template>
  <PartnerLayout>
    <div class="variation-create" data-cy="partner-variations-create">
      <v-row justify="center">
        <v-col cols="12" md="6">
          <v-card class="glass-card pa-6" rounded="xl">
            <h1 class="text-h4 font-weight-bold mb-6">New Variation</h1>
            
            <v-form ref="variationForm" @submit.prevent="saveVariation">
              <v-text-field
                v-model="form.name"
                label="Variation Name (e.g. Large, Red)"
                required
                variant="outlined"
              ></v-text-field>
              ...
              <v-text-field
                ...
                type="number"
                prefix="$"
                hint="Use positive for extra cost, negative for discount"
                persistent-hint
                variant="outlined"
                class="mt-4"
              ></v-text-field>
...
const saveVariation = async () => {
  ...
  // Placeholder
  setTimeout(() => {
    notifications.success('Variation created successfully');
